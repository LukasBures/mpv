...
def find_fundamental_matrix(img_left, img_right):
    ...
    # vytvorení matice A pro výpocet matice F
    A = list()
    ...
    # výpocet F pomoci SVD rozkladu
    U, s, V = np.linalg.svd(A)
    # vybrani vektoru V s nejmensi singularni hodnotou
    V = V[-1].conj().T;
    F = V.reshape(3,3).copy()
    # vypocet vysledne matice F
    while np.linalg.matrix_rank(F)!= 2:
        (U,D,V) = np.linalg.svd(F);
        F = np.dot(np.dot(U,np.diag([D[0], D[1], 0])),V);
    #denormalizace matice F
