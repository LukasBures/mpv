...
def normalize(matrix):
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix[0])):
            matrix[i][j] = int(((matrix[i][j]+mabs)/max_val))*255
    return matrix
...
#
# Sx = extend(Sx, 1)
# Sx5 = convolution(Sx, A)
#
# Sx5 = extend(Sx5, 1)
# Sx7 = convolution(Sx5, A)
#
# print(Sx7)
...
# # corners = absm(corners)
# # corners = normalize(corners)
# np.savetxt('numpy_out.txt', np.asarray(corners))
# plt.imshow(corners, cmap='gray')
# plt.show()
#
# print(imgHarris[30])
# print(np.asarray(corners[30]))
#
