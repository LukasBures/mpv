...
def kmeans(data, num_clusters, threshold=0.0001):
...
def kmeans_bin(data, num_clusters, threshold=0.0001):
    rand_indices = np.arange(0, len(data))
    np.random.shuffle(rand_indices)
...
def bow(train_data, train_label, test_data, n_visual_words):
    ...
    for i in range(len(train_data)):
        ...
        lengths[train_label[i]] += len(descs)
...
if __name__ == '__main__':
    ...
    cls = bow(imgsTrain, trainLabel, imgsTest, 11)
