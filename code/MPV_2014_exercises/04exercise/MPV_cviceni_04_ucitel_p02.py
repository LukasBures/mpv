...
if __name__ == '__main__':
    ...
    while(True):
        ...
        if((mode == 1) or (mode == 2)):
            ...
            if len(goodMatch) > 15:
                ptsTemplate = np.float32([kpTemplate[m.queryIdx].pt \
                                for m in goodMatch]).reshape(-1, 1, 2)
                ptsVideoImg = np.float32([kpVideoImg[m.trainIdx].pt \
                                for m in goodMatch]).reshape(-1, 1, 2)
            
                H, mask = cv2.findHomography(ptsTemplate, ptsVideoImg,
                                             cv2.RANSAC, 5.0)
                ...
                # Draw keypoits match
                matchesImg = drawMatches(Template, kpTemplate,
                                         Video, kpVideoImg, goodMatch)
        ...
        # ORB
            ...
            if(type(desVideo) == type(desTemplate)):
                matches = matcher.match(desTemplate, desVideo)
                matches = sorted(matches, key = lambda val: val.distance)
                matchesImg = drawMatches(Template, kpTemplate,
                                         Video, kpVideoImg, matches[:15])
