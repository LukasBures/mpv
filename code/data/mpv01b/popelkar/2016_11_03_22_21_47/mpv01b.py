...
def panorama(imgs, target_size):
    ...
    #vytvoreni listu pro while cyklus (osetreni preskocenych obrazu)
    ...
    while(sum(isDone) < len(imgs)-1):
        ...
        if isDone[tmp-1] == 0:
            ...
            if len(good) > 0:
                ...
                #nalezeni matice homografie, nastaveni RANSAC (jinak nefunguje spolehlive a obrazy jsou deformovane)
                ...
                #odecet od puvodniho obrazu
                odecet = cv2.subtract(img0Big, imgF)
                #spojeni obrazku dohromady (vyse uvedene dela cerne artefakty)
                img0Big = cv2.add(odecet,imgF)

                # pro vykresleni matchu mezi obrazy je nize uvedeny zakomentovany kod
                # iii = np.array([])
