...
def drawMatches(img1, kp1, img2, kp2, matches):
    ...
    for mat in matches:
        ...
        cv2.line(out, (int(x1), int(y1)), (int(x2) + cols1, int(y2)), (B, G, R),
                 1, cv2.CV_AA)
...
def drawlines(Img, lines, colors):
    ...
    for l in lines:
        ...
        pt1 = np.int32(pt1)
        pt2 = np.int32(pt2)
        cv2.line(Img, (pt1[0], pt1[1]), (pt2[0], pt2[1]), colors[n], 3, cv2.CV_AA)
        n += 1
...
if __name__ == '__main__':
    ...
    PT_l = np.float32([KP_l[m.queryIdx].pt for m in \
                        GoodMatches]).reshape(-1, 1, 2)
    PT_r = np.float32([KP_r[m.trainIdx].pt for m in \
                        GoodMatches]).reshape(-1, 1, 2)
    print "OK"
        
    #--------------------------------------------------------------------------
    # Visualization.
    print "Vizualization of keypoints matches ...",
    ...
    # Vybere jen body ktere byly pouzity pro vypocet.
    ...
    drawlines(Img_r, lines1[:nPt], colors)

    lines2 = cv2.computeCorrespondEpilines(PT_r, 2, F)
    drawlines(Img_l, lines2[:nPt], colors)

    for i in range(nPt):
        cv2.circle(Img_l, (int(PT_l[i, 0][0]), int(PT_l[i, 0][1])), \
                    10, colors[i], -1)   
        cv2.circle(Img_r, (int(PT_r[i, 0][0]), int(PT_r[i, 0][1])), \
                    10, colors[i], -1)   
