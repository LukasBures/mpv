...
def classify(train_data, train_label, test_data):
    ...
    for i in range(len(train_data)):
        temp = transform(cv2.cvtColor(train_data[i], cv2.COLOR_BGR2GRAY))
    ...
    svm.train(np.asarray(fv, dtype='float32'), cv2.ml.ROW_SAMPLE, np.asarray([labels], dtype='int32'))

    fvTest = list()
    for i in range(len(imgsTest)):
        fvTest.append(makeRowFromImg(imgsTest[i]))
