...
if __name__ == '__main__':
        ...
        if found:
            ...
            if VERBOSE:
                ...
                cv2.drawChessboardCorners(Vis, PatternSize, corners,
                                          found)
                cv2.imwrite(os.path.dirname(sys.argv[0]) + VerboseDir + str(n) + "_chessboard.jpg", Vis)
    ...
    print "------------------------------------------------------------------"
    rms, cameraMatrix, distCoefs, rvecs, tvecs = cv2.calibrateCamera(ObjPoints, ImgPoints, (w, h))
