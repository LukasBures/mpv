...
def TrainRandomForest(data, responses):
    rF = cv2.RTrees()
    print "--------------------------------------------------------"
    print "Trenovani Nahodneho lesa"
...
if __name__ == "__main__":
    clsPath = ["0", "1", "2", "3", "4"]
    clsTrainPath = "/data/train/"
    clsTestPath = "/data/test/"
    ...
    rF = TrainRandomForest(np.asarray(trainData, dtype = 'float32'),
    ...
    for i in range(len(clsPath)):
        ...
        for file in glob.glob("*.jpg"):
            ...
            predicted.append(rF.predict(np.asarray(feat,
                             dtype = 'float32')))
