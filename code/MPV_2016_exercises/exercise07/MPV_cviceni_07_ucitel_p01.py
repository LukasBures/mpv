...
Revision Note:
3.0.0 - 25.10.2016 - Updated for OpenCV 3.1.0 version
"""
...
def mean_shift(img, result_img):
    ...
    # HS hodnot v obrazku.
    hist, _, _ = np.histogram2d(imghsv[:, :, 1].ravel(), imghsv[:, :, 0].ravel(), [256, 256], [[0, 256], [0, 256]])
    ...
    # Cyklus, ktery je zastavem pokud neexistuje bod, ktery není zarazeny do nejake mnoziny.
    while True:

        # Inkrementace iterace a jeji vypsani do konzole.
        nth_iter += 1
        if nth_iter == 1:
            print "Number of iteration =", nth_iter, ",",
        else:
            print nth_iter, ",",
        ...
        # Aktualni bod stredu.
        bool_distance_all = np.zeros(len(dataarray), np.bool)

        # Cyklus dokud nedokonverguje stred.
        while True:

            # Vzdalenost vsech bodu od aktualniho stredu.
            distance = np.sqrt(np.sum(np.power(np.subtract(a_center, dataarray), 2), 1))

            # Ulozi True na pozici, pokud je bod v okenku kol
            # em aktualniho stredu.
            bool_distance = distance <= s_win_half

            # Logicky OR, uchovava informaci o vsech navstivenych bodech behem behem jedne cesty.
            bool_distance_all |= bool_distance

            # Vypocte vazeny soucet bodu, ktere jsou v aktualnim okenku.
            sumx = 0.0
            sumy = 0.0
            n_pt = 0.0
            for i in range(len(bool_distance)):
                if bool_distance[i]:
            ...
            a_center = np.array((sumx / n_pt, sumy / n_pt))

            # Zastavovaci podminka, pokud jsou polohy stredu ve dvou po sobe jdoucich iteracich stejne.
            if (old_center == a_center).all():

                # Pokud nemam ulozeny zadny stred.
                if len(bag_of_centers) == 0:

                    # Uloz [cislo clusteru, [stred clusteru X, Y]]
                    bag_of_centers.append([n_cluster, a_center])

                    # Uloz hlas shluku pro body, ktere byly cestou navstiveny.
                    for i in range(len(bool_distance_all)):
                        if bool_distance_all[i]:
                            cluster_vote[i].append(n_cluster)
                ...
                # Pokud jiz existuji nejake ulozene stredy shluku.
                    ...
                    for c in range(len(bag_of_centers)):
                        if (bag_of_centers[c][1] == a_center).all():
                            ...
                            for i in range(len(bool_distance_all)):
                                if bool_distance_all[i]:
                                    cluster_vote[i].append(bag_of_centers[c][0])
    ...
    # Vykresleni originalniho obrazku po shlukovani, kazdy shluk je reprezentovan barvou stredu shluku. Pro hodnotu
    # Value byla zvolena hodnota V viz vyse.
    img_result = cv2.cvtColor(img_result, cv2.COLOR_HSV2BGR)
    cv2.namedWindow("Result", cv2.WINDOW_NORMAL)
    cv2.imshow("Result", img_result)

    # Vykresleni originalniho obrazku po shlukovani, kazdy shluk je reprezentovan barvou stredu shluku. Pro hodnotu
    # Value byla zvolena hodnota originalniho obrazku.
