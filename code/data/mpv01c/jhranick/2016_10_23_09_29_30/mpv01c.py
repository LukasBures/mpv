...
def convolution(matrix, kernel, xshift=0,yshift=0):
    """
    ...
    matrix = extend_image(matrix, extd)
    print(extd)
    # print("i in <%d, %d>" % (extd, len(matrix) - extd))
    # print("j in <%d, %d>" % (extd, len(matrix[0]) - extd))
...
def sobel(img, axis, ksize):
    ...
    if axis == 'x':
        ...
        print(Sx)
        return convolution(img, Sx, yshift=1)
    elif axis == 'y':
        for i in range(int(ksize / 2) - 1):
            Sy = extend(Sy, 1)
            Sy = convolution(Sy, K)
        return convolution(img, Sy, xshift=1)
    else:
...
def harris(gsimg, block_size, ksize, k):
    ...
    Ixx = extend_image(Ixx, extd)
    Iyy = extend_image(Iyy, extd)
    Ixy = extend_image(Ixy, extd)
...
# imgGS = cv2.imread(r"D:\OneDrive\Faculty of Applied Science\Postgraduate Master Studies\2016-2017\MPV\Semester Projects Repository\SP1\c\sampleData\rect_tiny.jpg", cv2.IMREAD_GRAYSCALE)
#
# plt.figure(0)
...
# print(NormCorners[n])
#
# for n in range(len(imgGS)):
#     print(np.sum(NormHarris[n]-NormCorners[n]))
#
