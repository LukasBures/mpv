...
def k_means(trid, deskr):
    # deskr = np.array([[2 , 1 , 5],[ 1, -2 , 10],[ -5, 2, -3],[ 2, 5, 6],[ -10, 0 , 3],[5,-10,- 6],[5, 3, 2],[4, -2, -5]])
    # #deskr = np.array([[5, 3, 2, 5], [-1, 2, 5, 7], [-5, 4, 2, -7], [4, -2, -5, -6], [-4, 5, -4, 5]])
    # trid = 3
    p = 0
    stredy = list()
    for i in range(trid):
    ...
    old_stredy = np.zeros(trid) #stare stredy
    trida = list() #prislusnost desktriptoru do trid
    while True:
        if np.array_equal(old_stredy, stredy) or p > 15:
            break
        vzd = list() # vzdalenosti deskriptor? od stred?
        trida = list()
        ...
        for i in range(len(deskr)):
            ...
            pocet[trida[i]] += 1
        old_stredy = copy.deepcopy(stredy)
        for i in range(trid):
...
def bow(train_data, train_label, test_data, n_visual_words):
    ...
    for i in train_data: #vypocet SIFT deskriptoru pro trenovani
        a, des = sift.detectAndCompute(i, None)  #test
    ...
    trid = max(train_label)
    hist = np.zeros([ trid + 1, n_visual_words])
    for i in range(len(deskr)): #vypocet modelovych histogramu trid
        hist[lab[i] , trida[i] ] += 1
    print 'a'
    for i in range(trid + 1):
        a = sum(hist[i])
