...
\par{Cílem semestrální práce je vypočítat fundamentální matici pomocí 8--mi bodového algoritmu.}

\par{\textbf{Vstup:} Dvojice (levý a pravý) šedotónových obrázků, $0-255$, uint8.}

\par{\textbf{Výstup:} Vypočtená fundamentální matice o velikosti $3 \times 3$.}
...
\par{\textbf{Deadline:} V polovině ledna - konkrétní datum je na stránkách SAKa. Kdo bude chtít jít na zkoušku před deadline a~bude chtít zápočet typu $A$, nechť odevzdá semestrální práci \textbf{alespoň 3 dny před zkouškou a zašle e-mail cvičícímu}, aby byl čas na opravu.}
...
\begin{enumerate}
	\item Vypočítat klíčové body (například: SIFT, SURF, atd.). Najít korespondenty. Vybrat dobré páry pomocí \textsf{Lowe's ratio test} - s vhodně zvolenou konstantou (například 0.7).
	\item Vypočítat fundamentální matici pomocí 8--mi bodového algoritmu. Dát si pozor na normování ($\sqrt{2}$).
\end{enumerate}

\section*{Doporučená literatura}
\begin{enumerate}
	\item Přednášky a učební materiály MPV.
	\item The eight--point algorithm for the fundamental matrix.pdf
	\item In Defence of the 8--point Algorithm.pdf
\end{enumerate}
