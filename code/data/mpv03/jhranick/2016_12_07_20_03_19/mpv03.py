...
def compute_confusion(labels_gt, test_labels):
    """
    ...
    for i, j in zip(labels_gt, test_labels):
        confusion[i, j] += 1
...
def classify(train_data, train_label, test_data):
    ...
    # Get the features vector for each train image
    train_fv = []
    for i in range(len(train_data)):
        image = train_data[i]
