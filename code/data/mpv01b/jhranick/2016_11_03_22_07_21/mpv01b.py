...
def panorama(imgs, target_size):
    ...
    # pom = np.zeros((target_size[1], target_size[0], 3), dtype=np.uint8)
    # pom[:img_base.shape[0], :img_base.shape[1]] = img_base
    # plt.imshow(cv2.cvtColor(pom, cv2.COLOR_BGR2RGB), )
    # plt.show()
    # img_base = pom
    ...
    for i in range(len(imgs)):
        ...
        # pom = 0
        # for i in range(img_base_warp.shape[0]):
        #     for j in range(img_base_warp.shape[1]):
        #         if img_base[i, j, :].all() == 0 and img_next_warp[i, j, :].all() != 0:
        #             pom+=1
        #             img_base[i, j] = img_next_warp[i, j]
        # print(np.count_nonzero(img_next_warp), imgs[img_next['id']].shape)
        # exit(7)

        # plt.imshow(cv2.cvtColor(img_base_warp, cv2.COLOR_BGR2RGB), )
        # plt.show()
        # exit(7)
        # img_stitched = img_base
    ...
    plt.show()
    cv2.imwrite(r"D:\OneDrive\Faculty of Applied Science\Postgraduate Master Studies\2016-2017\MPV\Semester Projects Repository\SP1\b\sampleData\stitched_pano.png", img_stitched)
