...
mmsColor = cv2.imread("mms.jpg", cv2.IMREAD_COLOR)
...
#------------------------------------------------------------------------------
# Priklad 3: Nacteni a vykresleni sedotonoveho obrazku, vypocet histogramu
# a distribucni funkce.
#------------------------------------------------------------------------------
...
# Vykresleni legendy.
plt.legend(('Cumulative Distribution Function', 'Histogram'), 
           loc = 'upper left')
...
# Vytvori novou figuru, stejne jako v MATLABu.
h = plt.figure("Globally Equalized Histogram")
...
plt.legend(('Cumulative Distribution Function',
            'Globally Equalized Histogram'), loc = 'upper left')
...
# Vytvori novou figuru, stejne jako v MATLABu.
h = plt.figure("Contrast Limited Adaptive Histogram Equalization (CLAHE)")
...
# Zavre figuru s nazvem "Contrast Limited Adaptive Histogram Equalization
# (CLAHE)".
...
# Nacteni sedotonoveho obrazku.
ImgGS_rice = cv2.imread("rice.jpg", cv2.IMREAD_GRAYSCALE)
...
retval, ImgThOtsu = cv2.threshold(ImgCLAHE_rice, 0, 255,
                                  cv2.THRESH_OTSU)
...
# Provede 1 iteraci morfologicke operace otevreni s vyse definovanym
# strukturnim elementem. 
ImgThOtsuOpen = cv2.morphologyEx(ImgThOtsu, cv2.MORPH_OPEN,
                                 strElement, 1)

# Nalezne kontury.
contours, hirearchy = cv2.findContours(ImgThOtsuOpen, cv2.RETR_LIST, 
                                       cv2.CHAIN_APPROX_SIMPLE)

# Pokud je pocet kontur vetsi nez nula, tak se vykresli a vypise
# pocet zrn ryze na obrazku.
if len(contours) > 0:
    ...
    # Vykresli nalezene kontury do vytvoreneho obrazku, 3. parametr
    # -1 znamena, ze budou vykresleny vsechny kontury, cervenou
    # barvou (BGR) a 5. parametr znamena, ze kontury budou vykresleny
    # vyplnene.
    ...
    # Nyni se obrazek vykresli na nekonecne dlouhou dobu v [ms],
    # dokud nebude stisknuta libovolna klavesa.
    cv2.waitKey(0)
