...
def computeSift(images):
    ...
    for i in range(len(images)):
        ...
        # TODO: nemusi jich byt 125 - muze jich byt je 100 napriklad - nepotvrdilo se
        if len(one_sifts[0]) < 125:
            print "Sift count less than 125"
...
def getClassForPoints(points, centers):
	...
	for p in range(len(points)):
        # TODO: np.sqrt - chzbi, ale nema to na vypocet argmin vliv
		distances[p] = np.argmin(np.sqrt(np.sum(np.power(np.subtract(centers, points[p]), 2.0), 1.0)))
	return distances


def makeHistograms(points, clazz, count):
    # TODO: U vas - pocet histogramu nerovna se poctu trid do kterych klasifikujeme (10 != 5)
    # Histogram se dela z cetnosti viualnich slov v dane tride
    # Takze pro 10 vizualnich slov (centroidu K-means) pocitam pro kazdy FV vzdalenost k nejblizsimu centroidu
    # a do histogramu zanesu +1 na pozici vizualniho slova.
    # Takze by to melo vypadat 5 (trid) a kazda trida reprezentovana histogramem s 10 binama (poctu vizualnich slov)
...
def computeAngles(hist, centers_hist):
	...
	for i in range(len(centers_hist)):
		...
		aa = np.sum(u * v)
		b = np.sqrt(np.sum(np.power(u, 2)))
		c = np.sqrt(np.sum(np.power(v, 2)))
		alpha = np.arccos(aa / (b * c))
...
def bow(train_data, train_label, test_data, n_visual_words):
	...
	descriptors = getSiftDescriptors(test_sifts, [0] * len(test_data))
	# for i, d in enumerate(descriptors):
	# 	print i, " - ", d
