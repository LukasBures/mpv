...
# ------------------------------------------------------------------------------
def calculate_sif_descriptors(train_data, train_label):
    sift = cv2.SIFT(125)
...
def calculate_centroids(descriptors, n_visual_words):
    ...
    _, label, center = cv2.kmeans(descriptors, n_visual_words, criteria, attempts, cv2.KMEANS_PP_CENTERS)
...
def bow(train_data, train_label, test_data, n_visual_words):
    ...
    train_desc, n_desc_in_class = calculate_sif_descriptors(train_data, train_label)
